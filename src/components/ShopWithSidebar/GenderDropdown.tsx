...
const GenderItem = ({ category, isSelected, toggleGender }) => {
  return (
    <button
      className={`${
...
const GenderDropdown = ({ genders, selectedGenders, setSelectedGenders }) => {
  ...
  const handleToggleGender = (genderName: string) => {
  ...
  return (
    <div className="bg-white shadow-1 rounded-lg">
      ...
      >
        <p className="text-[12px] font-black uppercase text-[#212121] tracking-widest">Jenis Kelamin</p>
        <button
          onClick={() => setToggleDropdown(!toggleDropdown)}
      ...
      >
        {genders.map((gender, key) => (
          <GenderItem 
            key={key} 
            category={gender} 
            isSelected={selectedGenders.includes(gender.name)}
            toggleGender={() => handleToggleGender(gender.name)}
          />
        ))}
      </div>
    </div>
