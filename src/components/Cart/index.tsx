...
import { removeAllItemsFromCart, hydrateCart } from "@/redux/features/cart-slice";
...
const Cart = () => {
  const { isProfileComplete, session } = useAuth();
  const cartItems = useAppSelector((state) => state.cartReducer.items);
  const dispatch = useDispatch();
  const { clearAllCartSupabase, performGlobalSync } = useCartSync();
  ...
  useEffect(() => {
    ...
    initCart();
  }, [dispatch, performGlobalSync]);

  const [appliedCoupon, setAppliedCoupon] = useState<any>(null);
  const [appliedCode, setAppliedCode] = useState("");

  const handleApplyDiscount = useCallback((coupon: any, code: string) => {
    ...
    <>
      {loading ? (
        <div className="max-w-[1280px] w-full mx-auto px-4 sm:px-8 xl:px-6 2xl:px-0 py-10 lg:py-20 flex flex-col gap-6">
          ...
          <div className="flex flex-col lg:flex-row gap-7.5 mt-4">
            <Skeleton className="w-full lg:w-1/2 h-32" />
            <Skeleton className="w-full lg:w-1/2 h-48" />
          </div>
        </div>
      ) : (
        <>
      ...
      <section>
        <Breadcrumb title={"Keranjang"} pages={["Keranjang"]} />
      </section>
      ...
      {cartItems.length > 0 ? (
          ...
          <div className="max-w-[1280px] w-full mx-auto px-4 sm:px-8 xl:px-6 2xl:px-0">
            <div className="flex flex-col gap-6 mb-8">
              <div className="flex flex-wrap items-center justify-between gap-5">
                <h2 className="font-medium text-dark text-2xl">Keranjang Anda</h2>
                <button type="button" onClick={handleClearCart} className="hidden lg:block text-blue hover:text-red transition-colors duration-200">Kosongkan Keranjang Belanja</button>
              </div>

              {/* Mobile Discount */}
              <div className="lg:hidden">
                <Discount onApplyDiscount={handleApplyDiscount} appliedCode={appliedCode} onRemoveDiscount={handleRemoveDiscount} />
                <div className="mt-4 text-left">
                  <button type="button" onClick={handleClearCart} className="text-blue hover:text-red transition-colors duration-200 text-base font-medium">Kosongkan Keranjang Belanja</button>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-[10px] shadow-1 overflow-hidden">
              <div className="w-full">
                <div className="lg:min-w-[1170px]">
                  {/* <!-- table header --> */}
                  <div className="hidden lg:grid grid-cols-[1.5fr_1fr_1.2fr_1fr_0.5fr] items-center py-6 px-7.5 border-b border-gray-3 bg-gray-50">
                    <div className="text-left">
                      <p className="text-dark font-black text-xs uppercase tracking-widest opacity-80">Produk</p>
                    </div>

                    <div className="text-left">
                      <p className="text-dark font-black text-xs uppercase tracking-widest opacity-80">Harga</p>
                    </div>

                    <div className="text-left">
                      <p className="text-dark font-black text-xs uppercase tracking-widest opacity-80">Kuantitas</p>
                    </div>

                    <div className="text-left">
                      <p className="text-dark font-black text-xs uppercase tracking-widest opacity-80">Subtotal</p>
                    </div>

                    <div className="text-left">
                      <p className="text-dark font-black text-xs uppercase tracking-widest opacity-80">Aksi</p>
                    </div>
                  </div>
            ...
            <div className="flex flex-col lg:flex-row gap-7.5 xl:gap-11 mt-9">
              ...
              <OrderSummary appliedCoupon={appliedCoupon} appliedCode={appliedCode} />
            </div>
          </div>
        </section>
      ) : (
        <>
          <div className="text-center mt-8 px-4">
            ...
            <p className="pb-6">Keranjang Anda kosong!</p>

            <Link
              href="/shop-with-sidebar"
              className="w-full max-w-[400px] mx-auto flex justify-center font-medium text-white bg-blue py-[13px] px-6 rounded-md ease-out duration-200 hover:bg-blue-dark"
            >
              Lanjut Berbelanja
            </Link>
          </div>
