...
import { KODI_SIZE } from "@/utils/kodiPricing";
...
const ProductItem = ({ item, priority = false }: { item: Product; priority?: boolean }) => {
  ...
  // add to cart - Default to 1 Kodi (20 Pcs) for B2B
  const handleAddToCart = async () => {
    ...
    await syncCartItemToSupabase({ ...item, quantity: KODI_SIZE });
  };
  ...
  const handleItemToWishList = async () => {
    if (isWishlisted) {
      ...
      await syncItemToSupabase({ ...item, quantity: 1 }, true);
    }
  };
  ...
  const { calculateKodiPrice, formatRupiah } = require("@/utils/kodiPricing");
  const kodiPrice = calculateKodiPrice(item.discountedPrice || 0);
      ...
      <div className="relative aspect-square w-full bg-white rounded-[8px] mb-4">
        ...
        <div className="absolute inset-x-5 bottom-12 flex items-center justify-center gap-1 opacity-0 translate-y-3 ease-out duration-500 group-hover:opacity-100 group-hover:translate-y-0 z-20">
          ...
          <button
            ...
            className={`flex items-center justify-center w-10 h-10 shrink-0 rounded-full shadow-lg transition-all duration-300 border relative group/love ${
              isWishlisted 
                ? "bg-gradient-to-br from-red-light to-red text-white border-transparent" 
                : "bg-white/90 backdrop-blur-sm text-dark hover:text-red border-gray-3/30"
            }`}
          >
            <svg 
              width="18" 
              ...
              className={`transition-all duration-300 ${isWishlisted ? 'drop-shadow-sm' : 'group-hover/love:scale-110'}`}
            >
      ...
      <div className="p-4 text-left">
        ...
        <div className="flex flex-col gap-0.5 mb-3">
          <div className="flex items-baseline gap-1">
            <span className="text-blue font-bold text-[22px]">{formatRupiah(kodiPrice)}</span>
            <span className="text-[14px] text-blue font-semibold uppercase">/ KODI</span>
          </div>
          <div className="flex items-center gap-2 text-[14px]">
            {item.price > item.discountedPrice && (
              <span className="text-[#212121] line-through decoration-1">{formatRupiah(item.price * KODI_SIZE)}</span>
            )}
            {item.price > item.discountedPrice && (
              <span className="text-green font-bold">Hemat {formatRupiah((item.price - item.discountedPrice) * KODI_SIZE)}/Kodi</span>
            )}
          </div>
        </div>
