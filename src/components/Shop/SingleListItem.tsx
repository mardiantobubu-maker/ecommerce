...
const SingleListItem = ({ item, priority = false }: { item: Product; priority?: boolean }) => {
  ...
  return (
    <div className="group rounded-[10px] bg-white shadow-1 transition-all hover:shadow-xl overflow-hidden">
      <div className="flex flex-col sm:flex-row">
        <div className="shadow-list relative flex items-center justify-center sm:max-w-[270px] w-full sm:min-h-[270px] p-4 bg-white rounded-l-[10px]">
          <Link
        ...
        <div className="w-full flex flex-col gap-5 sm:flex-row sm:items-center justify-center sm:justify-between py-5 px-4 sm:px-7.5 lg:pl-11 lg:pr-12">
          <div>
            <h3 className="font-semibold text-[21px] text-dark ease-out duration-200 hover:text-blue mb-2">
            ...
            <div className="mt-4">
              <span className={`inline-flex items-center px-4 py-1.5 rounded-full text-[14px] font-bold ${item.stock && item.stock >= 20 ? 'bg-green/10 text-green' : 'bg-red/10 text-red'}`}>
                {item.stock && item.stock < 20 ? `Stok Terbatas: < 1 Kodi` : `Stok: ${Math.floor((item.stock || 0) / 20).toLocaleString("id-ID")} Kodi`}
              </span>
            </div>
          </div>
